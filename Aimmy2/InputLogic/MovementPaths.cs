...
namespace InputLogic
{
    class MovementPaths
    {
        ...
        // v2.6.5 body — EMA smoothing applied inside. Used by "None".
        internal static Point CubicBezier(Point start, Point end, Point control1, Point control2, double t)
        {
            ...
            if (MouseManager.IsEMASmoothingEnabled)
            {
                x = MouseManager.EmaSmoothing(MouseManager.previousX, x, MouseManager.smoothingFactor);
                y = MouseManager.EmaSmoothing(MouseManager.previousY, y, MouseManager.smoothingFactor);
            }

            return new Point((int)x, (int)y);
        }

        // v2.5.5 body — math only, no internal EMA. Used by "Cubic Bezier".
        internal static Point CubicBezierLegacy(Point start, Point end, Point control1, Point control2, double t)
        {
            double u = 1 - t;
            double tt = t * t;
            double uu = u * u;
